...
def train(
    ...
    for X_1, U, t_coll, time in train_loader:
        ...
        epoch_loss_ic += l_ic  
        if rollout: 
            l_roll, l_roll_phy = rollout_loss_fn(model, X_1, U, time, N_roll=20, device=device, t_coll=t_coll, pinn=pinn, noise_level=noise_level) 
            epoch_loss_roll += l_roll
        ...
        #combined_grad = operator.calculate_gradient(grads)
        
        # scale the physics gradient
        if pinn and rollout:
            g_0_norm = torch.norm(grads[0])
            g_1_norm = torch.norm(grads[1])
            g_2_norm = torch.norm(grads[2])
            g_3_norm = torch.norm(grads[3])
            scaling_factor_1 = g_0_norm / (g_1_norm + 1e-12)
            scaling_factor_2 = g_0_norm / (g_2_norm + 1e-12)
            scaling_factor_3 = g_0_norm / (g_3_norm + 1e-12)
            grads[1] = scaling_factor_1*grads[1] 
            grads[2] = scaling_factor_2*grads[2] 
            grads[3] = scaling_factor_3*grads[3]  
            
            combined_grad = 1.0*grads[0] + 0.5*grads[1] + 1.0*grads[2] + 0.5*grads[3] # g_data, g_phy, g_roll, g_roll_phy
            ...
            combined_grad = 1.0*grads[0] + 1.0*grads[1] # g_data, g_roll
            combined_grad = g_0_norm/torch.norm(combined_grad)*combined_grad
        elif pinn:
            g_0_norm = torch.norm(grads[0])
            g_1_norm = torch.norm(grads[1])
            scaling_factor_1 = g_0_norm / (g_1_norm + 1e-12)
            grads[1] = scaling_factor_1 * grads[1]
            combined_grad = 1.0 * grads[0] + 0.5 * grads[1]
            combined_grad = g_0_norm / torch.norm(combined_grad) * combined_grad
        else:
